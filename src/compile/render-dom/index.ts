...
export default function dom(
	...
	props.forEach(x => {
		...
		if (!variable.writable || component.component_options.accessors) {
			...
			body.push(deindent`
				get ${x.export_name}() {
					throw new Error("<${component.tag}>: Props cannot be read directly from the component instance unless compiling with 'accessors: true' or '<hamber:options accessors/>'");
				}
			`);
		}
		...
		if (component.component_options.accessors) {
			...
			body.push(deindent`
				set ${x.export_name}(value) {
					throw new Error("<${component.tag}>: Props cannot be set directly on the component instance unless compiling with 'accessors: true' or '<hamber:options accessors/>'");
				}
			`);
		}
	});
	...
	if (options.customElement) {
		...
		const superclass = options.dev ? 'HamberComponentDev' : 'HamberComponent';
