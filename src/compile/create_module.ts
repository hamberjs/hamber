...
export default function create_module(
	...
	if (format === 'esm') {
		return esm(code, name, banner, hamberPath, internal_path, helpers, imports, module_exports, source);
	}

	if (format === 'cjs') return cjs(code, name, banner, hamberPath, internal_path, helpers, imports, module_exports);

	throw new Error(`options.format is invalid (must be ${list(Object.keys(wrappers))})`);
}

function edit_source(source, hamberPath) {
	return source === 'hamber' || source.startsWith('hamber/')
		? source.replace('hamber', hamberPath)
		: source;
}
	...
	const user_imports = imports.length > 0 && (
		imports
			.map((declaration: Node) => {
				const import_source = edit_source(declaration.source.value, hamberPath);
...
function cjs(
	...
	const requires = imports.map(node => {
		...
		const source = edit_source(node.source.value, hamberPath);
