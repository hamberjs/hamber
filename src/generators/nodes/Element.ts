	...
	build(
		...
		if (this._cssRefAttribute) {
			block.builders.hydrate.addLine(
				`@setAttribute(${name}, "hamber-ref-${this._cssRefAttribute}", "");`
			)
		}
		...
		if (allUsedContexts.size || eventHandlerUsesComponent) {
			...
			allUsedContexts.forEach((contextName: string) => {
				...
				updates.push(
					`${name}._hamber.${listName} = state.${listName};\n${name}._hamber.${indexName} = state.${indexName};`
				);
			});
		...
		function toHTML(node: Element | Text) {
			...
			if (node._cssRefAttribute) {
				open += ` hamber-ref-${node._cssRefAttribute}`;
			}
		...
		groups.forEach(group => {
			...
			block.builders.init.addBlock(deindent`
				function ${handler}() {
					...
					${usesContext && `var context = ${this.var}._hamber;`}
		...
		this.attributes.filter((a: EventHandler) => a.type === 'EventHandler').forEach((attribute: EventHandler) => {
			...
			const declarations = usedContexts
				.map(name => {
					...
					if (contextType === 'each') {
						...
						return `var ${listName} = ${ctx}._hamber.${listName}, ${indexName} = ${ctx}._hamber.${indexName}, ${contextName} = ${listName}[${indexName}];`;
					}
				})
				.filter(Boolean);
			...
			const handlerBody = deindent`
				${eventHandlerUsesComponent &&
					`var ${block.alias('component')} = ${ctx}._hamber.component;`}
				${declarations}
				...
				if (generator.options.dev) {
					block.builders.hydrate.addBlock(deindent`
						if (${handlerName}.teardown) {
							console.warn("Return 'destroy()' from custom event handlers. Returning 'teardown()' has been deprecated and will be unsupported in Hamber 2");
						}
					`);
				}
