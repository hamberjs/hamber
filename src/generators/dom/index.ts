...
export default function dom(
	...
	const generator = new DomGenerator(parsed, source, options.name || 'HamberComponent', stylesheet, options);
	...
	if (sharedPath) {
		...
		usedHelpers.forEach(key => {
			...
			if (key === 'transitionManager') {
				// special case
				const global = `_hamberTransitionManager`;
	...
	return generator.generate(result, options, {
		banner: `/* ${filename ? `${filename} ` : ``}generated by Hamber v${"__VERSION__"} */`,
		sharedPath,
		helpers,
		name,
		format,
