...
export class MappedCode {
	...
	static from_source({ source, file_basename, get_location }: Source): MappedCode {
		...
		// we know that it will eventually be merged with hamber's map,
...
export function apply_preprocessor_sourcemap(filename: string, hamber_map: SourceMap, preprocessor_map_input: string | DecodedSourceMap | RawSourceMap): SourceMap {
	if (!hamber_map || !preprocessor_map_input) return hamber_map;
	...
	const result_map = combine_sourcemaps(
		filename,
		[
			hamber_map as RawSourceMap,
			preprocessor_map
		]
	) as RawSourceMap;

	// Hamber expects a SourceMap which includes toUrl and toString. Instead of wrapping our output in a class,
