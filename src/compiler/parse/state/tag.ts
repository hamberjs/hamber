...
const valid_meta_tags = Array.from(meta_tags.keys()).concat('hamber:self', 'hamber:component');
	...
	if (name === 'hamber:component') {
		...
		if (!~index) {
			parser.error({
				...
				message: `<hamber:component> must have a 'this' attribute`
			}, start);
		}
...
function read_tag_name(parser: Parser) {
	...
	if (parser.read(SELF)) {
		...
		if (!legal) {
			parser.error({
				...
				message: `<hamber:self> components can only exist inside {#if} blocks, {#each} blocks, or slots passed to components`
			}, start);
		}
	...
	if (name.startsWith('hamber:')) {
		...
		let message = `Valid <hamber:...> tag names are ${list(valid_meta_tags)}`;
