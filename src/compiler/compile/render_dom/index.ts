...
export default function dom(
	...
	props.forEach(prop => {
		...
		if (!variable.writable || component.component_options.accessors) {
			...
			accessors.push({
				...
				key: { type: 'Identifier', name: prop.export_name },
				value: x`function() {
					throw new @_Error("<${component.tag}>: Props cannot be read directly from the component instance unless compiling with 'accessors: true' or '<hamber:options accessors/>'");
				}`
			});
		}
		...
		if (component.component_options.accessors) {
			...
			accessors.push({
				...
				key: { type: 'Identifier', name: prop.export_name },
				value: x`function(value) {
					throw new @_Error("<${component.tag}>: Props cannot be set directly on the component instance unless compiling with 'accessors: true' or '<hamber:options accessors/>'");
				}`
			});
		}
	});
	...
	if (options.customElement) {
		...
		const declaration = b`
			class ${name} extends @HamberElement {
				constructor(options) {
					super();
		...
		const superclass = {
			type: 'Identifier',
			name: options.dev ? '@HamberComponentDev' : '@HamberComponent'
		};
		...
		const declaration = b`
			class ${name} extends ${superclass} {
				constructor(options) {
					...
					${options.dev && b`@dispatch_dev("HamberRegisterComponent", { component: this, tagName: "${name.name}", options, id: create_fragment.name });`}
