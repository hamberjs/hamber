...
export default class Component {
	...
	constructor(
		...
		if (compile_options.customElement) {
			if (
				...
				const hamberOptions = ast.html.children.find(
					child => child.name === 'hamber:options'
				) || { start: 0, end: 0 };
				this.warn(hamberOptions, {
					code: 'custom-element-no-tag',
					message: `No custom element 'tag' option was specified. To automatically register a custom element, specify a name with a hyphen in it, e.g. <hamber:options tag="my-thing"/>. To hide this warning, use <hamber:options tag={null}/>`,
				});
			}
	...
	generate(result?: { js: Node[]; css: CssResult }) {
		...
		if (result) {
			...
			const banner = `${this.file ? `${this.file} ` : ``}generated by Hamber v${'__VERSION__'}`;
		...
		for (let i = 0; i < body.length; i += 1) {
			...
			if (node.type === 'VariableDeclaration') {
				const all_hoistable = node.declarations.every(d => {
					...
					// everything except const values can be changed by e.g. hamber devtools
...
function process_component_options(component: Component, nodes) {
	...
	const node = nodes.find(node => node.name === 'hamber:options');
				...
				switch (name) {
					...
					default:
						component.error(attribute, {
							...
							message: `<hamber:options> unknown attribute`,
						});
				}
			} else {
				component.error(attribute, {
					code: `invalid-options-attribute`,
					message: `<hamber:options> can only have static 'tag', 'namespace', 'accessors', 'immutable' and 'preserveWhitespace' attributes`,
				});
			}
		});
	}
