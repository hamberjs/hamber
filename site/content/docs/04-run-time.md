...
The `hamber` package exposes [lifecycle functions](/tutorial/onmount) and the [context API](/tutorial/context-api).
...
```hb
<script>
	import { beforeUpdate } from 'hamber';
...
```hb
<script>
	import { beforeUpdate, tick } from 'hamber';
...
```hb
<script>
	import { getAllContexts } from 'hamber';
...
The `hamber/store` module exports functions for creating [readable](/docs#run-time-hamber-store-readable), [writable](/docs#run-time-hamber-store-writable) and [derived](/docs#run-time-hamber-store-derived) stores.

Keep in mind that you don't *have* to use these functions to enjoy the [reactive `$store` syntax](/docs#component-format-script-4-prefix-stores-with-$-to-access-their-values) in your components. Any object that correctly implements `.subscribe`, unsubscribe, and (optionally) `.set` is a valid store, and will work both with the special syntax, and with Hamber's built-in [`derived` stores](/docs#run-time-hamber-store-derived).

This makes it possible to wrap almost any other reactive state handling library for use in Hamber. Read more about the [store contract](/docs#component-format-script-4-prefix-stores-with-$-to-access-their-values-store-contract) to see what a correct implementation looks like.
...
```js
import { derived } from 'hamber/store';
...
```js
import { readonly } from 'hamber/store';
...
The `hamber/motion` module exports two functions, `tweened` and `spring`, for creating writable stores whose values change over time after `set` and `update`, rather than immediately.
...
* `easing` (`function`, default `t => t`) — an [easing function](/docs#run-time-hamber-easing)
...
Out of the box, Hamber will interpolate between two numbers, two arrays or two objects (as long as the arrays and objects are the same 'shape', and their 'leaf' properties are also numbers).
...
As with [`tweened`](/docs#run-time-hamber-motion-tweened) stores, `set` and `update` return a Promise that resolves if the spring settles.
...
The `hamber/transition` module exports seven functions: `fade`, `blur`, `fly`, `slide`, `scale`, `draw` and `crossfade`. They are for use with Hamber [`transitions`](/docs#template-syntax-element-directives-transition-fn).
...
* `easing` (`function`, default `linear`) — an [easing function](/docs#run-time-hamber-easing)
...
* `delay` (`number`, default 0) — milliseconds before starting
* `duration` (`number`, default 400) — milliseconds the transition lasts
* `easing` (`function`, default `cubicInOut`) — an [easing function](/docs#run-time-hamber-easing)
...
```hb
<script>
	import { blur } from 'hamber/transition';
</script>
...
* `delay` (`number`, default 0) — milliseconds before starting
* `duration` (`number`, default 400) — milliseconds the transition lasts
* `easing` (`function`, default `cubicOut`) — an [easing function](/docs#run-time-hamber-easing)
...
<script>
	...
	import { quintOut } from 'hamber/easing';
</script>
...
```hb
<script>
	import { slide } from 'hamber/transition';
	import { quintOut } from 'hamber/easing';
</script>
...
```hb
<script>
	import { scale } from 'hamber/transition';
	import { quintOut } from 'hamber/easing';
</script>
...
```hb
<script>
	import { draw } from 'hamber/transition';
	import { quintOut } from 'hamber/easing';
</script>
...
```hb
<script>
	import { crossfade } from 'hamber/transition';
	import { quintOut } from 'hamber/easing';
...
The `hamber/animate` module exports one function for use with Hamber [animations](/docs#template-syntax-element-directives-animate-fn).
...
Easing functions specify the rate of change over time and are useful when working with Hamber's built-in transitions and animations as well as the tweened and spring utilities. `hamber/easing` contains 31 named exports, a `linear` ease and 3 variants of 10 different easing functions: `in`, `out` and `inOut`.
...
To render Hamber components in Node.js without bundling, use `require('hamber/register')`. After that, you can use `require` to include any `.hamber` file.

```js
require('hamber/register');

const App = require('./App.hamber').default;
...
```js
require('hamber/register')({
  extensions: ['.customextension'], // defaults to ['.html', '.hamber']
...
const app = new App({
	...
	props: {
		// assuming App.hamber contains something like
		// `export let answer`:
		answer: 42
	}
});
...
The `hydrate` option instructs Hamber to upgrade existing DOM (usually from server-side rendering) rather than creating new elements. It will only work if the component was compiled with the [`hydratable: true` option](/docs#compile-time-hamber-compile). Hydration of `<head>` elements only works properly if the server-side rendering code was also compiled with `hydratable: true`, which adds a marker to each element in the `<head>` so that the component knows which elements it's responsible for removing during hydration.

Whereas children of `target` are normally left alone, `hydrate: true` will cause any children to be removed. For that reason, the `anchor` option cannot be used alongside `hydrate: true`.

The existing DOM doesn't need to match the component — Hamber will 'repair' the DOM as it goes.
...
Hamber components can also be compiled to custom elements (aka web components) using the `customElement: true` compiler option. You should specify a tag name for the component using the `<hamber:options>` [element](/docs#template-syntax-hamber-options).

```hb
<hamber:options tag="my-element" />
...
```js
import MyElement from './MyElement.hamber';
...
To prevent this, add `accessors={false}` to `<hamber:options>`.
...
Custom elements can be a useful way to package components for consumption in a non-Hamber app, as they will work with vanilla HTML and JavaScript as well as [most frameworks](https://custom-elements-everywhere.com/). There are, however, some important differences to be aware of:
...
* In Hamber, slotted content renders *lazily*. In the DOM, it renders *eagerly*. In other words, it will always be created even if the component's `<slot>` element is inside an `{#if ...}` block. Similarly, including a `<slot>` in an `{#each ...}` block will not cause the slotted content to be rendered multiple times
...
A server-side component exposes a `render` method that can be called with optional props. It returns an object with `head`, `html`, and `css` properties, where `head` contains the contents of any `<hamber:head>` elements encountered.

You can import a Hamber component directly into Node using [`hamber/register`](/docs#run-time-hamber-register).
