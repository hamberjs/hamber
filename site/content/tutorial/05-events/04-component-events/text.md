...
Components can also dispatch events. To do so, they must create an event dispatcher. Update `Inner.hamber`:
...
> You can also try changing the event name to something else. For instance, change `dispatch('message')` to `dispatch('myevent')` in `Inner.hamber` and change the attribute name from `on:message` to `on:myevent` in the `App.hamber` component.
