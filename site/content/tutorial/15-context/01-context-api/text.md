...
Let's set the context first. In `Map.hamber`, import `setContext` from `hamber` and `key` from `mapbox.js` and call `setContext`:

```js
import { onDestroy, setContext } from 'hamber';
...
On the other side of the equation, in `MapMarker.hamber`, we can now get a reference to the Mapbox instance:
