...
describe('sourcemaps', () => {
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		(solo ? it.only : skip ? it.skip : it)(dir, async () => {
			...
			const inputFile = path.resolve(`${__dirname}/samples/${dir}/input.hamber`);
			...
			js.code = js.code.replace(
				/generated by Hamber v\d+\.\d+\.\d+/,
				match => match.replace(/\d/g, 'x')
			);

			fs.writeFileSync(`${outputBase}.hamber`, preprocessed.code);
			if (preprocessed.map) {
				fs.writeFileSync(
					`${outputBase}.hamber.map`,
					// TODO encode mappings for output - hamber.preprocess returns decoded mappings
			...
			if (js.map) {
				assert.deepEqual(
					...
					(config.js_map_sources || ['input.hamber']).sort(),
			...
			if (css.map) {
				assert.deepEqual(
					...
					(config.css_map_sources || ['input.hamber']).sort(),
