...
describe('runtime (puppeteer)', () => {
	before(async () => {
		...
		console.log('[runtime-puppeteer] Loaded Hamber');
				...
				plugins: [
					{
						...
						resolveId(importee) {
							if (importee === 'hamber/internal' || importee === './internal') {
								return internal;
							}
						...
						load(id) {
							if (id === 'main') {
								return `
									import HamberComponent from ${JSON.stringify(
										path.join(__dirname, 'samples', dir, 'main.hamber')
									)};
						...
						transform(code, id) {
							if (id.endsWith('.hamber')) {
								const compiled = hamber.compile(code.replace(/\r/g, ''), {
								...
								const out = `${out_dir}/${path.basename(id).replace(/\.hamber$/, '.js')}`;
