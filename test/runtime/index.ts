...
const hamberPath = process.cwd().split('\\').join('/');
...
describe('runtime', () => {
	before(() => {
		...
		hamber = loadHamber(false);
		hamber$ = loadHamber(true);

		require.extensions['.hamber'] = function(module, filename) {
	...
	function runTest(dir, hydrate, from_ssr_html) {
		...
		(config.skip ? it.skip : solo ? it.only : it)(testName, (done) => {
			...
			compile = (config.preserveIdentifiers ? hamber : hamber$).compile;

			const cwd = path.resolve(`${__dirname}/samples/${dir}`);

			compileOptions = config.compileOptions || {};
			compileOptions.format = 'cjs';
			compileOptions.hamberPath = hamberPath;
			...
			glob('**/*.hamber', { cwd }).forEach(file => {
				if (file[0] === '_') return;

				const dir = `${cwd}/_output/${hydrate ? 'hydratable' : 'normal'}`;
				const out = `${dir}/${file.replace(/\.hamber$/, '.js')}`;
			...
			Promise.resolve()
				.then(() => {
					...
					try {
						mod = require(`./samples/${dir}/main.hamber`);
						HamberComponent = mod.default;
					} catch (err) {
					...
					window.HamberComponent = HamberComponent;
					...
					if (hydrate && from_ssr_html) {
						...
						cleanRequireCache();
						const SsrHamberComponent = require(`./samples/${dir}/main.hamber`).default;
						const { html } = SsrHamberComponent.render(config.props);
					...
					const component = new HamberComponent(options);
				...
				.catch(err => {
					...
					err.stack += `\n\ncmd-click: ${path.relative(process.cwd(), cwd)}/main.hamber`;
					done(err);
					throw err;
				})
				.then(() => {
	...
	async function create_component(src = '<div></div>') {
		...
		const bundle = await rollup({
			...
			plugins: [
				...
				{
					name: 'hamber-packages',
					resolveId: (importee) => {
						if (importee.startsWith('hamber/')) {
							return importee.replace('hamber', process.cwd()) + '/index.mjs';
						}
					}
				}
			]
