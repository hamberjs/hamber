...
import { hamber, loadConfig, tryToLoadJson } from '../helpers';
...
describe('validate', () => {
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		(solo ? it.only : skip ? it.skip : it)(dir, () => {
			...
			const input = fs.readFileSync(`${__dirname}/samples/${dir}/input.hamber`, 'utf-8').replace(/\s+$/, '').replace(/\r/g, '');
			...
			try {
				const { warnings } = hamber.compile(input, {
					dev: config.dev,
	...
	it('warns if options.name is not capitalised', () => {
		const { warnings } = hamber.compile('<div></div>', {
			name: 'lowercase',
			generate: false
		});
