...
describe('ssr', () => {
	before(() => {
		compile = loadHamber(true).compile;
	...
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		(solo ? it.only : it)(dir, (done) => {
			...
			try {
				...
				const Component = require(`${dir}/main.hamber`).default;
				...
				err.stack += `\n\ncmd-click: ${path.relative(process.cwd(), dir)}/main.hamber`;
				done(err);
			} finally {
		...
		fs.readdirSync(`test/${suite}/samples`).forEach(dir => {
			...
			(config.skip ? it.skip : solo ? it.only : it)(dir, () => {
				...
				try {
					...
					const Component = require(`../${suite}/samples/${dir}/main.hamber`).default;
