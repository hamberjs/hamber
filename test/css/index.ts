...
import { assert, env, hamber, setupHtmlEqual, shouldUpdateExpected } from '../helpers';
...
function create(code) {
	...
	fn(module, module.exports, id => {
		if (id === 'hamber') return require('../../index.js');
		if (id.startsWith('hamber/')) return require(id.replace('hamber', '../../'));
...
describe('css', () => {
	...
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		(solo ? it.only : skip ? it.skip : it)(dir, () => {
			...
			const input = fs
				.readFileSync(`${__dirname}/samples/${dir}/input.hamber`, 'utf-8')
				.replace(/\s+$/, '')
				.replace(/\r/g, '');

			const expected_warnings = (config.warnings || []).map(normalize_warning);

			const dom = hamber.compile(
				input,
				Object.assign(config.compileOptions || {}, { format: 'cjs' })
			);

			const ssr = hamber.compile(
				input,
			...
			const actual_css = dom.css.code.replace(/hamber(-ref)?-[a-z0-9]+/g, (m, $1) => $1 ? m : 'hamber-xyz');
			try {
			...
			if (expected.html !== null) {
				...
				try {
					...
					const actual_html = html.replace(/hamber(-ref)?-[a-z0-9]+/g, (m, $1) => $1 ? m : 'hamber-xyz');
				...
				// ssr
				try {
					const actual_ssr = ServerComponent.render(config.props).html.replace(/hamber(-ref)?-[a-z0-9]+/g, (m, $1) => $1 ? m : 'hamber-xyz');
