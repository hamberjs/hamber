...
import { hamber, tryToLoadJson } from '../helpers';
...
describe('parse', () => {
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		const skip = !fs.existsSync(`${__dirname}/samples/${dir}/input.hamber`);
		...
		(skip ? it.skip : solo ? it.only : it)(dir, () => {
			...
			try {
				const { ast } = hamber.compile(input, Object.assign(options, {
					generate: false
				}));
