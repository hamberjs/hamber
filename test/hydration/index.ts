...
const hamberPath = process.cwd();

describe('hydration', () => {
	before(() => {
		const hamber = loadHamber();
		...
		require.extensions['.hamber'] = function(module, filename) {
			...
			const { js } = hamber.compile(fs.readFileSync(filename, 'utf-8'), options);
	...
	function runTest(dir) {
		...
		(config.skip ? it.skip : solo ? it.only : it)(dir, () => {
			...
			try {
				...
				const HamberComponent = require(`${cwd}/main.hamber`).default;
				...
				const component = new HamberComponent({
					target,
					hydrate: true,
					props: config.props
				});
